...
%msn_net = cmsn_network(cnn_training_data, training_label_cnn,17); 
...
cmsn_net_mlps = cmsn(final_label, 9, layer_per_stage,first_stage_input, ...
...
%the input for mst 
%predictedLabels_cmsn = cmsn_testing(first_stage_test, cmsn_net_mlps,17*3);

%cmsn_accuracy = sum(predicted_label==test_label)/numel(test_label); 
